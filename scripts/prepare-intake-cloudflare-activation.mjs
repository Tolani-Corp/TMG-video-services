    ...
    if (response.status !== 429) {
      if (!response.ok || lastBody?.success === false) {
        const details = Array.isArray(lastBody?.errors)
          ...
          : "";
        throw new Error(`Cloudflare API ${init.method || "GET"} ${path} failed HTTP ${response.status}${details ? `: ${details}` : ""}`);
      }
      return lastBody;
    }
    await new Promise((resolve) => setTimeout(resolve, attempt * 5_000));
  }
  throw new Error(`Cloudflare API ${init.method || "GET"} ${path} remained rate limited after retries (HTTP ${lastStatus}; ${JSON.stringify(lastBody)})`);
}
...
const evidence = {
  schema: "tmg.intake-cloudflare-preparation.v1",
  ...
  zone: { name: zoneName, id: zone.id, status: zone.status },
  access: {
